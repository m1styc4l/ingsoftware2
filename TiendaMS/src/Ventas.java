...
public class Ventas extends JFrame implements ActionListener{
	...
	public Ventas(Configuracion pConfiguracion,Cliente cliente) {
		JPanel p=new JPanel();
	...
	public void totalizar() {
		...
		String l = JOptionPane.showInputDialog(null,"1 para Efectivo"+"\n"+"2 para Cuotas");
		String[] txt = configuracion.leerVenta().get(configuracion.leerVentas().size()-1).split(";");
		int cons = Integer.parseInt(txt[0]);
		if(l.equals("1")) {
			Venta v = new Venta(cons+1, clientem.getCedula(), totaliza,configuracion.getTienda().getValorIVA(), totaiva );
			configuracion.agregarVenta(v);
			for (int i = 0; i < array.size(); i++) {
				Detalle_venta dv = new Detalle_venta(array.get(i).getCodigo_producto(), 
						cantidad.get(i), array.get(i).getPrecio_venta_producto(),
						array.get(i).getPrecio_venta_producto()*cantidad.get(i) , cons+1);
				configuracion.agregarVentas(dv);
			}
			JOptionPane.showMessageDialog(null, "Venta hecha");
		}else {
			JOptionPane.showMessageDialog(null, totaliza+"\n"+"Tasa de interes: "+configuracion.getTienda().getTasaInteres()+
					"\n"+"Plazo de 6 meses");
			configuracion.agregarCredito(new Credito(cons+1, clientem.getCedula()));
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(evento.equalsIgnoreCase("Totalizar")) {
			...
			if(buscarProducto(codigo)) {
				String canti = JOptionPane.showInputDialog(null, "Digite lacantidad:");
